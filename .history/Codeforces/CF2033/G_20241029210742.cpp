...
struct SegmentTree {
    ...
    Info rangeQuery(int p, int pl, int pr, int L, int R) {
        ...
        int mid = pl + pr >> 1;
        pushDown(p);
        return rangeQuery(p << 1, pl, mid, L, R) + rangeQuery(p << 1 | 1, mid, pr, L, R);
    }
...
void solve() {
    ...
    function<void(int, int)> dfs2 = [&](int u, int fa) {
        for (auto & [k, id] : querys[u]) {
            ...
            ans[id] = max(ans[id], seg.rangeQuery(max(0ll, dep[u] - k), dep[u]).maxn + dep[u]);
        }
        int max1{}, max2{};
        for (auto v : g[u]) {
            if (v == fa) continue;
