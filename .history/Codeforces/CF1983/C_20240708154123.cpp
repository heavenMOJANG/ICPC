...
void solve(){
    ...
    while(sufC[lR]<border&&lR>=1) lR--;
    if(rL<=lR&&preA[lR]-preA[rL-1]>=border){cout<<rL<<" "<<lR<<" 1 "<<rL-1<<" "<<lR+1<<" n\n";return;}
    //Situation 4 BCA
    while(preB[rL]<border&&rL<=n) rL++;
    while(sufA[lR]<border&&lR>=1) lR--;
    if(rL<=lR&&preC[lR]-preC[rL-1]>=border){cout<<lR+1<<" "<<n<<" 1 "<<rL-1<<" "<<rL<<" "<<lR<<"\n";return;}
    //Situation 5 CAB
    while(preC[rL]<border&&rL<=n) rL++;
    while(sufB[lR]<border&&lR>=1) lR--;
    if(rL<=lR&&preA[lR]-preA[rL-1]>=border){cout<<rL<<" "<<lR<<" "<<lR+1<<" n 1"<<rL-1<<"\n";return;}
    //Situation 6 CBA
    while(preC[rL]<border&&rL<=n) rL++;
    while(sufA[lR]<border&&lR>=1) lR--;
    if(rL<=lR&&preB[lR]-preB[rL-1]>=border){cout<<lR+1<<" n "}
    cout<<"-1\n";
    return;
}
signed main(){
