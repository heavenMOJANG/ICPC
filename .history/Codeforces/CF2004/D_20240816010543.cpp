...
void solve() {
    ...
    auto calc = [&](int u, int v, const vector<int> & b) {
        int res = INF;
        int p = upper_bound(b.begin(), b.end(), u) - b.begin();
        if (p != b.end() - b.begin()) res = abs(b[p] - u) + abs(v - b[p]);
        int p = lower_bound(b.begin(), b.end(), v) - b.begin() - 1;
        if  (p >= 0) res = abs(b[p] - u) + abs(v - b[p]);
        return res;
    };
    while (q --) {
        int u, v; cin >> u >> v;
