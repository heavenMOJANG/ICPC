...
void split(int & u, int & v, int pl, int pr, int L, int R) {
    if (pr < L || R < pl) return;
    if (!u) return;
    if (L <= pl && pr <= R) {
        v = u;
    ...
    if (!v) v = _new();
    int mid = pl + pr >> 1;
    if (L <= mid) split(t[u].l, t[v].l, pl, mid, L, R);
    if (R > mid) split(t[u].r, t[v].r, mid + 1, pr, L, R);
    pushUp(u);
    pushUp(v);
}
int query(int p, int pl, int pr, int L, int R) {
    ...
    if (L <= mid) res += query(t[p].l, pl, mid, L, R);
    if (R > mid) res += query(t[p].r, mid + 1, pr, L, R);
}
signed main() {
