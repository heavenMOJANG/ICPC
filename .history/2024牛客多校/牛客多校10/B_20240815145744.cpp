...
void solve() {
    ...
    int idx = 0;
    function<void(string, string)> build = [&](string str, string op) {
        if (op.size() == 0) {
            cout << str << "\n";
            return;
        }
        char o = str.front(); op.erase(0, 1);
        if (str[0] == 'p') {
            str.pop_back();
            ...
            switch (str[0]) {
                ...
                default:{
                    ...
                    string ls = str.substr(0, p + 1), rs = str.substr(p + 2, str.size() - 1);
                    build( ls);
                    build(rs);
                    break;
                }
    ...
    function<void(int, string)> dfs = [&](int u, string str) {
        ...
        char op = str.front(); str.erase(0, 1);
