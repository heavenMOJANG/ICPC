...
void disexc(char *str){
	...
	unsigned char *tp=find(str);
	if (tp!=NULL){
		unsigned short length = *(tp + 5) << 8 | *(tp + 4);
		unsigned short currentBLock = *(tp + 7) << 8 | *(tp + 6);
		unsigned char *p = filecon;
		while (currentBLock != 0xFFFF && length > 0) {
			int size = (length > SectorSize) ? SectorSize : length;
			readdisk(p, currentBLock, size);
			p += size;
			length -= size;
...
int main(){
	...
	while(1){
		printf("\n>");
		getline(inputs,sizeof inputs, stdin);
		shell(inputs);
		menu();
	}
    return 0;
