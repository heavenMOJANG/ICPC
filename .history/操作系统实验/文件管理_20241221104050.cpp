...
//功能：显示str目录内容
//str：可以采用绝对或相对路径
bool direxc(const char *str) {
    int i, j;
    const char *dirstr[2] = {"<file>", "<dir>"};
    unsigned char *p = (unsigned char *)Dir; // 根目录指针
    char filename[4]; // 文件名缓冲
    ...
    for (i = 0; i < Maxfilecount; i++) {
        if (*p != '\0') { // 如果目录项非空
            ...
            start = (start << 8) + *(p + 6);
            // 打印目录项
            printf("%6s %6s %5u %5u\n", filename, dirstr[*(p + 3) == 0xFF], length, start);
        }
        p += 8; // 每个目录项占 8 字节
    }
    // 显示剩余磁盘空间
...
bool mdexc(char *str) {
    unsigned char *tmp = findfirstemptyDir(); // 查找第一个空目录项
    if (tmp != NULL) {
        writefilename(tmp, str); // 写入目录名到目录项
        *(tmp + 3) = 0xFF;       // 设置类型为目录（0xFF 表示目录）
        *(tmp + 4) = 0;          // 目录长度为 0
        *(tmp + 5) = 0;
        *(tmp + 6) = 0;          // 起始块号为 0
        *(tmp + 7) = 0;
...
void rdexc(char *str) {
    unsigned char *tp = find(str); // 查找目录项
    if (tp != NULL && *(tp + 3) == 0xFF) { // 确保是目录
        // 检查目录是否为空
        bool isEmpty = true;
        for (int i = 0; i < Maxfilecount; i++) {
            unsigned char *entry = Dir + i * 8;
            if (*entry != '\0' && strncmp((char *)entry, str, 3) == 0 && *(entry + 3) != 0xFF) {
                isEmpty = false; // 目录中还有文件或子目录
                break;
            }
        }
        if (isEmpty) {
