...
long long manhattan_distance(const Point& p1, const Point& p2) {
...
struct PQ_Edge {
    ...
    bool operator>(const PQ_Edge& other) const {
        return dist > other.dist;
    }
};
...
int main() {
    ...
    // Using a flag and regenerating the edge list is simpler to implement
    // and can pass if the number of Type 2 queries is not too high.
    ...
    for (int q = 0; q < Q; ++q) {
        ...
        if (type == 1) {
            ...
            // Merge all components with distance k
            if (min_dist != -1) {
