...
int check(itn x0, int y0, int x1, int y1) {
...
struct Node {
    ...
    bool operator > (const Node &rhs) const {
        ...
        if (dx == 2 && dy == 2) d = x + y + 4;
        if (rdx == 1 && rdy == 1) rhd = rhs.x + rhs.y;
        if (rdx == 1 && rdy == 2) rhd = rhs.x + rhs.y + 1;
        if (rdx == 2 && rdy == 1) rhd = rhs.x + rhs.y + 1;
        if (rdx == 2 && rdy == 2) rhd = rhs.x + rhs.y + 4;
        return d == rhd ? x == rhs.x ? y < rhs.y : x < rhs.x : d < rhd;
    }
};
void solve() {
    int n; cin >> n;
    ...
    int d = ceil(sqrt(n));
    priority_queue<Node, vector<Node>, greater<Node>> pq0;
    int sx = 1, sy = 1;
    for (int i = 0; i < d; ++ i)
        for (int j = 0; j < d; ++ j)
