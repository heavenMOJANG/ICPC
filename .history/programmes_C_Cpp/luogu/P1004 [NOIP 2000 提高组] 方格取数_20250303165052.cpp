...
int dfs(int u, int t, int flow = INF) {
    ...
    for (int &i = cur[u]; ~i && used < flow; i = e[i].nxt) {
        ...
        if (!vis[v] && e[i].cap && dis[v] == dis[u] + e[i].cst) {
            ...
            if (w) ret += w * e[i].cst, e[i].cap -= w, e[i ^ 1].cap += w, used += w;
        }
    }
    vis[u] = 0;
    return used;
...
signed main() {
    ...
    s = 0, t = n * n + 1;
    addEdge(s, 1, 2, 0); addEdge(n * n + 100, t, 2, 0);
    for (int i = 1; i <= n; ++ i)
        for (int j = 1; j <= n; ++ j) {
            int p = (i - 1) * n + j;
            addEdge(p, p + 100, 1, -a[i][j]);
            addEdge(p, p + 100, INF, 0);
            if (j < n) addEdge(p + 100, p + 1, INF, 0);
            if (i < n) addEdge(p + 100, p + n, INF, 0);
        }
    cout << mcmf() << "\n";
    return 0;
}