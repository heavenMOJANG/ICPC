...
void solve() {
    ...
    sort(h, h + N, [&](int a, int b){ return h[a].tot == h[b].tot ? dsu.fa[a] < dsu.fa[b] : h[a].tot > h[b].tot; });
    cout << hos.size() << "\n";
    for (auto x : hos) {
        cout << setw(10) << setfill('0') << dsu.fa[x] << " " << h[x].popu << " " << setprecision(3) << h[x].tot << " " << h[x].area << "\n";
    }
    return;
}
signed main() {
