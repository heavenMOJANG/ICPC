...
void solve() {
    ...
    sort(v.begin(), v.end(), [&](int a, int b) { return h[a].area == h[b].area ? a < b : h[a].area > h[b].area; });
    cout << hos.size() << "\n";
    for (int i = 0; i < v.size(); ++ i)
        cout << setw(4) << setfill('0') << dsu.fa[v[i]] << " " << h[v[i]].popu << " " << fixed << setprecision(3) << h[v[i]].tot << " " << h[v[i]].area << "\n";
    return;
}
signed main() {
