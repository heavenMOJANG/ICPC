...
void solve() {
    ...
    dis[1][1][2] = t[1][1];
    dis[1][1][3] = t[1][1] + d[1][1];
    if (n * m == 1) { cout << dis[1][1][3] << "\n"; return; }
    int ans = 1e18;
    pq.emplace(dis[1][1][2], 1, 1, 2);
    pq.emplace(dis[1][1][3], 1, 1, 3);
    while (!pq.empty()) {
        auto [curDis, x, y, curDir] = pq.top(); pq.pop();
        if (vis[(x - 1) * n + y + n * m * curDir]) continue;
        vis[(x - 1) * n + y + n * m * curDir] = 1;
        for (int k = 0; k < 4; ++ k) {
            int tx = x + dx[k], ty = y + dy[k];
            if (tx < 1 || tx > n || ty < 1 || ty > m) continue;
            int w = dis[tx][ty][curDir] + t[tx][ty] + (curDir != k) * d[tx][ty];
            if (w < dis[tx][ty][k]) {
                dis[tx][ty][k] = w;
