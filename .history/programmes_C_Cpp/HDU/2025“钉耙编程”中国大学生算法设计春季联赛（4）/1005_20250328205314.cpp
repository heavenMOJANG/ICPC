...
void solve() {
    ...
    int lend = D.size(), lenr = R.size();
    sort(D.begin(), D.end());
    sort(R.begin(), R.end(), greater<double>());
    vector<double> prd(D.size() + 1, 1.0l), prr(R.size() + 1, 0.0l);
    for (int i = 1; i <= D.size(); ++ i) prd[i] = prd[i - 1] * (D[i - 1] / 10.0l);
    for (int i = 1; i <= R.size(); ++ i) prr[i] = prr[i - 1] + R[i - 1] * 1.0l;
    double ans = P;
    for (int i{}; i <= k; ++ i) {
        if (i > D.size()) continue;
        int t = k - i;
        if (t > R.size()) continue;
        double tmp = max(0.0l, P * prd[i] - prr[t]);
        ans = min(ans, tmp);
    }
