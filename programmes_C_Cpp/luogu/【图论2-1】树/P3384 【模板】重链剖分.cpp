...
ll dx[]={-1,1,0,0};
ll dy[]={0,0,-1,1};
struct Edge{ll to,nxt;}edge[2*N];
ll n,m,r,mod;
ll head[2*N],idx,w[N],w_new[N],tree[N<<2],tag[N<<2];
void add_edge(ll u,ll v){
    edge[idx]={v,head[u]};
    head[u]=idx++;
}
void add_tag(ll k,ll l,ll r,ll d){        
    tag[k]+=d;
    tree[k]=(tree[k]+d*(r-l+1))%mod;
}
void push_up(ll k){tree[k]=(tree[k<<1]+tree[k<<1|1])%mod;}
...
void update(ll L,ll R,ll k,ll l,ll r,ll d){
...
ll son[N],id[N],fa[N],dep[N],siz[N],top[N];
...
void update_range(ll u,ll v,ll z){
    while(top[u]!=top[v]){
...
void update_tree(ll u,ll k){update(id[u],id[u]+siz[u]-1,1,1,n,k);}
ll query_tree(ll u){return query(id[u],id[u]+siz[u]-1,1,1,n)%mod;}
