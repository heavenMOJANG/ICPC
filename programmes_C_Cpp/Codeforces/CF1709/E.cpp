...
void solve() {
    ...
    map<int, int>cnt;
    function<void(int)> add = [&](int u) {cnt[dis[u]] ++;};
    function<void(int)> del = [&](int u) {cnt[dis[u]] --;};
    function<int(int, int)> check = [&](int u, int f) {return cnt[dis[u] ^ a[f]] ? 0 : 1;};
    ...
    function<void(int, int, int)> dfs1 = [&](int u, int f, int keep) {
        ...
        add(u);
        if (!keep || flag[u]) 
            for (int i = dfn[u]; i <= ctr[u]; ++ i) cnt.clear();
    };
    dfs0(1, 0);
    dfs1(1, 0, 1);
