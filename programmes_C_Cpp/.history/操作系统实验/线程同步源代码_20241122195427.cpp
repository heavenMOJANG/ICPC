...
HANDLE hHandle = NULL; // 主线程与子线程通信信号量
HANDLE hHandle1 = NULL; // full信号量的句柄，全局变量 
HANDLE hHandle2 = NULL; // empty信号量的句柄，全局变量     
...
{  
    ...
    hHandle1 = CreateSemaphore(NULL, 0, 10, NULL); // 创建full信号量, 初值为0
    if(hHandle1 == NULL)  
        cout << "full Semaphore Create Fail!" << endl;
    else 
        cout << "full Semaphore Create Success!" << endl;

    hHandle2 = CreateSemaphore(NULL, 10, 10, NULL); // 创建empty信号量，初值为10
    if(hHandle2 == NULL)  
