...
void solve(){
    ...
    for(int i=1;i<=n;++i){
        if(s[i]=='<'){
            ...
            if(cnt1[i-1]<cnt2[i+1]){
                int k=cnt1[i-1],num1=pre[i-1],num2;
                if(cnt2[i+1]-k-1-1==-1) num2=suf[i+1];
                else num2=suf[i+1]-suf[pos2[cnt2[i+1]-k-1-1]];
                ans[i]=2*num2-2*num1-i;
            }
            else if(cnt1[i-1]>cnt2[i+1]){
                int k=cnt2[i+1],num1=pre[i-1]-pre[pos1[cnt1[i-1]-k-1]],num2=suf[i+1];
                ans[i]=n+1-i+2*num2-2*num1;
            }
            else{
                int k=cnt2[i+1],num1=pre[i-1],num2=suf[i+1];
                ans[i]=n+1-i+2*num2-2*num1;
            }
        }
    }
