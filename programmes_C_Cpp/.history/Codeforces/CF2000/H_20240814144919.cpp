    ...
    while (m --) {
        ...
        switch (op) {
            ...
            case '-':
                ...
                break;
            case '?':
                auto f = [&](const Info & info) {return info.ans >= x;};
                int pos = seg.findFirst(1, N, f);
                if (pos == N + 1) pos = *prev(s.end()) + 1;
                else pos = pos - x + 1;
                cout << pos << " ";
        }
    }
    cout << "\n";
    for (auto x : s) seg.modify(x, {1, 1, 1, 1});
}
signed main() {
