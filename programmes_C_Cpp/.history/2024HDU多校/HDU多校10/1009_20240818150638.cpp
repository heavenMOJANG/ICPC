...
void solve() {
    ...
    string ba = b.substr(nxt2[lenAB], lenB) + a;
    string ab = a.substr(nxt1[lenAB], lenA) + b;
    int cntAB_a = kmp(a, ab), cntAB_b = kmp(b, ab);
    int cntBA_a = kmp(a, ba), cntBA_b = kmp(b, ba);
    if (cntAB_a == cntAB_b) ans = min(ans, lenAB - nxt1[lenAB]);
    if (cntBA_a == cntBA_b) ans = min(ans, lenAB - nxt2[lenAB]);
    cout << ans << "\n";
}
signed main() {
