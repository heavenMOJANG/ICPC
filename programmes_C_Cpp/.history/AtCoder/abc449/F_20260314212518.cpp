...
struct S { int maxn, len; };
using F = long long;
S op(S a, S b) {
    if (a.maxn != b.maxn) return a.maxn < b.maxn ? a : b;
    return {a.maxn, a.len + b.len};
}
S e() { return {(int)4e18, 0ll}; }
S mapping(F f, S x) { return {x.maxn + f, x.len}; }
...
void solve() {
    ...
    auto cal = [&]() {
        ...
        return sum - (tmp.maxn == 0 ? tmp.len : 0ll);
    };
    int res{}, px = v[0].x;
