...
void solve() {
    ...
    for (int i{}; i < q; ++ i) cin >> qe[i].first, qe[i].second = i;
    sort(qe.begin(), qe.end(), [](const auto& x, const auto& y) { return x.first < y.first; });
    vector<int> ans(q);
    int l{}, r{};
    while (l < q && r < seg.size()) {
        int x = qe[l].first;
        auto s = seg[r];
        if (s.L <= x && x <= s.R) ans[qe[l ++].second] = s.A ? x + s.B : s.B;
        else if (x > s.R) r ++;
        else ans[qe[l ++].second] = 0;
    }
    while (l < q) {
        ans[qe[l].second] = seg.back().A ? qe[l].first + seg.back().B : seg.back().B;
        l ++;
    }
