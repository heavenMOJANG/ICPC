...
int query(int x, int y, segtree<S, op, e> &seg) {
    int res{};
    while (top[x] != top[y]) {
        if (dep[top[x]] < dep[top[y]]) swap(x, y);
        res += seg.prod(dfn[top[x]] - 1, dfn[x]);
        x = fa[top[x]];
    }
    if (dep[x] > dep[y]) swap(x, y);
    res += seg.prod(dfn[x] - 1, dfn[y]);
    return res;
}
void solve() {
    cin >> n;
    ...
    for (int mex{}; mex < n; mex ++) {
        ...
        if (dr > d) l = mex, d = dr;
        int mid = kth(l, r, d / 2);
        int cl = query(l, mid, seg) - seg.get(dfn[mid] - 1);
        int cr = query(mid, r, seg) - seg.get(dfn[mid] - 1);
        ans += cl * cr;
        int pos = dfn[mex] - 1;
