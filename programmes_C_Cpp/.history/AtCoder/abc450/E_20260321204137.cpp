...
void solve() {
    string X, Y; cin >> X >> Y;
    int Q; cin >> Q;
    vector<int> l = {0ll, (int)X.length(), (int)Y.length()};
    ...
    function<int(int, int, char)> cal = [&](int n, int p, char c) {
        if (p <= 0) return 0ll;
        if (n == 1) {
            p = min(p, (int)X.length());
            int res{};
            for (int i{}; i < p; ++ i) res += (X[i] == c);
            return res;
        }
        if (n == 2) {
            p = min(p, (int)Y.length());
            int res{};
            for (int i{}; i < p; ++ i) res += (Y[i] == c);
            return res;
        }
        if (p <= l[n - 1]) return cal(n - 1, p, c);
        return cnt[n - 1][c - 'a'] + cal(n - 2, p - l[n - 1], c);
    };
    int m = l.size() - 1;
    while (Q --) {
        int L, R; cin >> L >> R;
