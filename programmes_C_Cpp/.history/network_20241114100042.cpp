...
void initialization() {
    ...
    if (LOBYTE(wsadata.wVersion) != 2 || HIBYTE(wsadata.wHighVersion) != 2) {
        std::cerr << "SocketVersion not matched!" << std::endl;
        WSACleanup();
        exit(EXIT_FAILURE);
    }
    std::cout << "Initialization Succeeded! SocketVersion matched!" << std::endl;
}
...
int main() {
    ...
    server_addr.sin_addr.S_un.S_addr = inet_addr("127.0.0.1"); // 允许从任何IP地址连接
    ...
    if (listen(s_server, SOMAXCONN) < 0) {
        std::cerr << "Setting listen state failed!" << std::endl;
        closesocket(s_server);
        WSACleanup();
        return -1;
    }

    std::cout << "Server is listening on port 16384..." << std::endl;
